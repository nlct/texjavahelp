...
help frame buttons. \Figureref{fig:navbuttons} shows the four navigation
buttons: home (go to the start of the manual), previous page (go to
the previous section), up (go to parent section), and next page (go
to the next section). Note that \qt{page} in this context refers to
the file displayed in the help window, which typically contains a
section, and doesn't relate to the page numbers in the \gls{pdf}.

\begin{figure}
\centering
\includegraphics{images/navbuttons}
\caption{Navigation Buttons}
\label{fig:navbuttons}
\end{figure}

There are also history buttons, which allow the user to go back to
previously viewed pages or, if they have previously gone back, to go
forward. \Figureref{fig:historybuttons} shows the history buttons.
